...
@Code
    ...
    Dim checkboxvisibility = "hidden" '"visible" '"hidden"
    Dim x = Me.Culture
    Dim xscale As Double = 0.7
    Dim yscale As Double = 0.7
    Dim myattsmodel As New List(Of KeyValuePair(Of String, String))
    Dim vwmodelname = Model.ViewModel.GetType.Name
    myattsmodel.Add(New KeyValuePair(Of String, String)("vwmodelname", vwmodelname))
    Dim pclemsdsid = Model.SDS_ID
    myattsmodel.Add(New KeyValuePair(Of String, String)("pcelemsdsid", pclemsdsid))
    Dim pclemguidstr = Model.MyGuid.ToString
    Dim xxxxx = Model.ToRGBAColor(Model.ViewModel.Background)
    Dim pcelembackground = xxxxx 'Model.ViewModel.Background.Color.ToString.Remove(1, 2)
    ...
    If TypeOf Model.ViewModel Is MvcApplication2.QuestOptionSetModel Then
        With CType(Model.ViewModel, MvcApplication2.QuestOptionSetModel)
            ...
            'If Not .DesignerItemWidth > 0 Or Not .DesignerItemHeight > 0 Then
            '    xpcelemwidth = .UniGridColumns * qwidth
            '    xpcelemheight = .UniGridRows * qheight
            'End If
            Dim qheight = xpcelemheight / Math.Max(1, .UniGridRows)
            Dim qwidth = xpcelemwidth / Math.Max(1, .UniGridColumns) - layoutmarginwidth
            'Dim pctqheight = (qheight / 768) * 100
            'Dim pctqwidth = (qwidth / 1366) * 100
            'Dim spctqheight = PageContentElement.ToPct(pctqheight)
            'Dim spctqwidth = PageContentElement.ToPct(pctqwidth)
    ...
    'myattsmodel.Add(New KeyValuePair(Of String, String)("pcelembackground", pcelembackground))
    'myattsmodel.Add(New KeyValuePair(Of String, String)("pcelemheight", pcelemheight))
    'myattsmodel.Add(New KeyValuePair(Of String, String)("pcelemwidth", pcelemwidth))
    'myattsmodel.Add(New KeyValuePair(Of String, String)("pcelemDILeft", pcelemDILeft))
    'myattsmodel.Add(New KeyValuePair(Of String, String)("pcelemDITop", pcelemDITop))
    'myattsmodel.Add(New KeyValuePair(Of String, String)("pcelemDIZindex", pcelemDIZindex))
    'myattsmodel.Add(New KeyValuePair(Of String, String)("pcelemoffset", pcelemDILeft & " " & pcelemDITop))
    Dim xdioffsetleft = Html.Raw(Json.Encode(pcelemDILeft))
    Dim xdioffsettop = Html.Raw(Json.Encode(pcelemDITop))
    'Dim myModel = Html.Raw(Json.Encode(myattsmodel))
...
@*<div style="position:relative;height:@pcelemheight;width:@pcelemwidth;background-color:@(pcelembackground);"*@
...
@*<script language="javascript" type="text/javascript">
    (function (prweb, $) {
        $('div[data-prwpcelmsdsid=@pclemsdsid]').pcelementview({myxheight: @(xheight), myxwidth: @(xwidth), myoffset:@(xdioffset), myoffsetleft:@(xdioffsetleft), myoffsettop:@(xdioffsettop)});
    }(this.prweb, jQuery));
</script>*@
