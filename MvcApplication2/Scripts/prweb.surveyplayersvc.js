...
(function (prweb, $) {
    prweb.surveyplayersvc = (function () {
        var sps = {};
        sps.ModeTo = false;
        sps.NewPlayerSvc = function () {
            var that = {};
            ...
            that.DefaultPcelemColxn = function () {
                var rslt = [];
                return rslt;
            };
                                        ...
                                        success: function (pcmodel) {
                                            try {
                                                ...
                                                if (pageentry.pgnum == that.actionContext.currentpgnumber.toString()) {
                                                    var curndx = that.ToNdx(that.actionContext.currentpgnumber);
                                                    prweb.flex.ScaleEditables("#surveyplayersurveypage" + curndx.toString());
                                                }
                                                rslt = mpo;
                                            }
                                            catch (ex) {
                ...
                sapi.sizethePagesPanel = function () {
                    ...
                    try {
                        ...
                        if ($item.length == 1) {
                            ...
                            alert("surveyplayersvc.sapi.sizethePagesPanel says $item.length <> 1");
                        }
                    }
                    catch (ex) {
                    ...
                    that.scrollapi.$items.css("left", fxleft).animate({ left: oppamt }, { duration: 300, easing: 'linear', complete: function () {
                        if (isnewpage) {
                            ...
                            if (prweb.appmessagesvc !== undefined) {
                                prweb.appmessagesvc.PopulateViewMessage("Page " + that.actionContext.currentpgnumber.toString());
                            }
                        }
                        return;
                ...
                try {
                    ...
                    var tx = setTimeout(function () {
                        try {
                            ...
                            //                    prweb.flex.ScaleEditables();
                            if (prweb.surveymodelsvc.ManagePageChange !== undefined) {
                                prweb.surveymodelsvc.ManagePageChange(parseInt(that.actionContext.currentpgnumber));
                            }
                        }
                        catch (ex) {
                            alert("prweb.surveyplayersvc.ManagePageChange.tx reports " + ex.Message);
                        }
                        clearTimeout(tx);
                        tx = null;
                        return;
